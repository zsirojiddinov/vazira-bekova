...
| 2.2 | Ot | S | **mustaqil hisoblangan** | stub: kodga faqat docx o'zagi berildi, natijani qoida zanjiri hisobladi | a: topilmadi; network: lug'at[json] |
| 2.3 | Ot | S | **mustaqil hisoblangan** | stub: kodga faqat docx o'zagi berildi, natijani qoida zanjiri hisobladi | an: topilmadi; example: lug'at[json] |
...
| 2.51 | Fe'l | L | **CH2_EVX_EXAMPLES orqali aylanma** | haqiqiy lug'at: natija FAQAT CH2_EVX_EXAMPLES yozuvidan (docx misoli bilan bir manba) | may: lug'at[chapter2_evx] |
| 2.52 | Fe'l | L | **CH2_EVX_EXAMPLES orqali aylanma** | haqiqiy lug'at: natija FAQAT CH2_EVX_EXAMPLES yozuvidan (docx misoli bilan bir manba) | might: lug'at[chapter2_evx] |
| 2.56 | Fe'l | S | **mustaqil hisoblangan** | stub: kodga faqat docx o'zagi berildi, natijani qoida zanjiri hisobladi | to: lug'at[docx]; ask: lug'at[json] +MDB almashtirdi |
...
| 2.62 | Fe'l | S | **mustaqil hisoblangan** | stub: kodga faqat docx o'zagi berildi, natijani qoida zanjiri hisobladi | will: lug'at[chapter2_evx]; return: topilmadi |
...
| 2.2 | Ot | **TO'LIQ MOS** | [S] _INDEFINITE_ARTICLE_UZ:2217 — noaniq artikl a/an — keyin ot kelsa, ot iborasiga "bitta" bo'lib kiradi — stub: `a network` → «Bitta tarmoq» (kutilgan «bitta tarmoq») ✓ |
| 2.3 | Ot | **TO'LIQ MOS** | [S] _INDEFINITE_ARTICLE_UZ:2217 — noaniq artikl a/an — keyin ot kelsa, ot iborasiga "bitta" bo'lib kiradi — stub: `an example` → «Bitta misol» (kutilgan «bitta misol») ✓ |
| 2.4 | Ot | **TO'LIQ MOS** | [S] _DETERMINERS:2213 — aniq artikl "the" va ko'rsatish so'zlarini (this/that ...) iboradan olib tashlaydi — stub: `the progress` → «Taraqqiyot» (kutilgan «taraqqiyot») ✓ |
...
| 2.51 | Fe'l | **TO'LIQ MOS** | [L] real lug'at: `may` → «mumkin» (kutilgan «mumkin») ✓ — yo'l: may: lug'at[chapter2_evx] |
| 2.52 | Fe'l | **TO'LIQ MOS** | [L] real lug'at: `might` → «mumkin» (kutilgan «mumkin») ✓ — yo'l: might: lug'at[chapter2_evx] |
| 2.53 | Fe'l | **QISMAN MOS** | [L] real lug'at: `must` → «shart» (kutilgan «kerak») ✗ — yo'l: must: lug'at[chapter2_evx] |
| 2.54 | Fe'l | **QISMAN MOS** | [L] real lug'at: `ought to` → «Ga zarur» (kutilgan «zarur») ✗ — yo'l: ought: lug'at[chapter2_evx] |
| 2.55a | Fe'l | **YO'Q** | [L] real lug'at: `need` → «[need?]» (kutilgan «kerak») ✗ — yo'l: need: topilmadi |
| 2.56 | Fe'l | **TO'LIQ MOS** | [S] INFINITIVE_PARTICLE_EN:2222 — "to" + fe'l — infinitiv yuklamasi, tarjima qilinmaydi (spec 2.56) — stub: `to ask` → «So‘ramoq» (kutilgan «so‘ramoq») ✓ |
...
| 2.59 | Fe'l | **TO'LIQ MOS** | [S] PREP_OBJECT_VERBS:2228 — (listen, to): predlog tushadi, to'ldiruvchi vositasiz; me/him/us obyekt shakli (OBJECT_CASE_PRONOUNS) qayta "-ni" olmaydi — stub: `listen to me` → «Meni tinglamoq» (kutilgan «meni tinglamoq») ✓ |
| 2.61 | Fe'l | **TO'LIQ MOS** | [L] real lug'at: `understand` → «Tushunmoq» (kutilgan «tushunmoq») ✓ — yo'l: understand: lug'at[json] |
| 2.62 | Fe'l | **TO'LIQ MOS** | [S] FUTURE_AUX_EN:2243 — "will" + fe'l → fe'lning o'zi (spec 2.62 misoli) — stub: `will return` → «Qaytmoq» (kutilgan «qaytmoq») ✓ |
...
| 2.2 | TO'LIQ MOS | `a network` → «Tarmoq» ✗ | a: topilmadi; network: lug'at[json] |
| 2.3 | TO'LIQ MOS | `an example` → «Bitta Misol» ✓ | an: topilmadi; example: lug'at[json] |
...
| 2.56 | TO'LIQ MOS | `to ask` → «Ajratib ko'rsatmoq» ✗ | to: lug'at[docx]; ask: lug'at[json] +MDB almashtirdi |
| 2.55b | QISMAN MOS | `reading` → «o'qishni» ✓ | reading: lug'at[chapter2_evx] |
| 2.59 | TO'LIQ MOS | `listen to me` → «Meniga» ✗ | listen: topilmadi; to: lug'at[docx]; me: lug'at[json] |
| 2.62 | TO'LIQ MOS | `will return` → «keladi [return?]» ✗ | will: lug'at[chapter2_evx]; return: topilmadi |
...
|---|---|---|---|---|---|---|---|
| 1 | 2.10 | Ot | YO'Q | N | — | 0 | `man → men` ⟹ `erkak → erkak + lar = erkaklar` |
| 2 | 2.36 | Fe'l | YO'Q | L | — | 0 | `read` ⟹ `o‘qimoq` |
| 3 | 2.38 | Fe'l | YO'Q | L | — | 0 | `to be` ⟹ `bo‘lmoq` |
| 4 | 2.39 | Fe'l | YO'Q | L | — | 0 | `am` ⟹ `man (1-shaxs birlik affiksi)` |
| 5 | 2.41 | Fe'l | YO'Q | L | — | 0 | `was, were` ⟹ `edi` |
| 6 | 2.44 | Fe'l | YO'Q | L | — | 0 | `to have` ⟹ `bor bo‘lmoq` |
| 7 | 2.45 | Fe'l | YO'Q | L | — | 0 | `to do` ⟹ `qilmoq` |
| 8 | 2.48 | Fe'l | YO'Q | L | — | 0 | `become` ⟹ `bo‘lmoq` |
...
- stub: `a network` → «Bitta tarmoq» (kutilgan «bitta tarmoq») ✓ — yo'l: translate_phrase; a: topilmadi; network: lug'at[stub]
- real lug'at: `a network` → «Tarmoq» (kutilgan «bitta tarmoq») ✗ — yo'l: translate_phrase; a: topilmadi; network: lug'at[json]
...
- stub: `an example` → «Bitta misol» (kutilgan «bitta misol») ✓ — yo'l: translate_phrase; an: topilmadi; example: lug'at[stub]
- real lug'at: `an example` → «Bitta Misol» (kutilgan «bitta misol») ✓ — yo'l: translate_phrase; an: topilmadi; example: lug'at[json]
...
- Tur: **M**, holat: **TO'LIQ MOS**
- Stub: busy→band (Sifat)
- stub: `busiest` → «Eng band» (kutilgan «eng band») ✓ — yo'l: translate_phrase; busiest: qoida -iest (o'zak busy, Sifat)
...
- Spec: `may` → `mumkin`
- Tur: **L**, holat: **TO'LIQ MOS**
- real lug'at: `may` → «mumkin» (kutilgan «mumkin») ✓ — yo'l: so'zma-so'z; may: lug'at[chapter2_evx]

### 2.52 — Modal fe’l “might” – “may”dan kamroq ehtimolni bildiradi

- Spec: `might` → `mumkin`
- Tur: **L**, holat: **TO'LIQ MOS**
- real lug'at: `might` → «mumkin» (kutilgan «mumkin») ✓ — yo'l: so'zma-so'z; might: lug'at[chapter2_evx]
...
- Spec: `ought to` → `zarur`
- Tur: **L**, holat: **QISMAN MOS**
- real lug'at: `ought to` → «Ga zarur» (kutilgan «zarur») ✗ — yo'l: translate_phrase; ought: lug'at[chapter2_evx]; to: lug'at[docx]
...
- stub: `to ask` → «So‘ramoq» (kutilgan «so‘ramoq») ✓ — yo'l: translate_phrase; to: lug'at[seed(kod)]; ask: lug'at[stub]
- real lug'at: `to ask` → «Ajratib ko'rsatmoq» (kutilgan «so‘ramoq») ✗ — yo'l: translate_phrase; to: lug'at[docx]; ask: lug'at[json] +MDB almashtirdi
...
- stub: `listen to me` → «Meni tinglamoq» (kutilgan «meni tinglamoq») ✓ — yo'l: translate_phrase; listen: lug'at[stub]; to: lug'at[seed(kod)]; me: lug'at[stub]
...
- Izoh: Spec (kanonik) o'zbekchasi "qaytmoq"; dissertatsiya II bobida xuddi shu misol "qaytadi" (reports/ch2_leakage_check.md) — ikki manba farq qiladi, spec'ga amal qilindi.
- stub: `will return` → «Qaytmoq» (kutilgan «qaytmoq») ✓ — yo'l: translate_phrase; will: topilmadi; return: lug'at[stub]
...
- real lug'at: `me` → «Meni» (kutilgan «meni») ✓ — yo'l: translate_phrase; me: lug'at[json]
- real lug'at: `him` → «Uni» (kutilgan «uni») ✓ — yo'l: translate_phrase; him: lug'at[json]
- real lug'at: `us` → «Bizni» (kutilgan «bizni») ✓ — yo'l: translate_phrase; us: lug'at[json]
